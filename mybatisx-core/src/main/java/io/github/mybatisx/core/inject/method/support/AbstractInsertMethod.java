 ...
 * @since 1.0.0
 */
public abstract class AbstractInsertMethod<T extends Supplier> extends AbstractInvokeMethod<T> {

    @Override
    public MappedStatement injectMappedStatement(Class<?> mapperInterface, Class<?> resultType, Table table) {
        return addInsertMappedStatement(mapperInterface, resultType, table);
    }
}
