 ...
 * @since 1.0.0
 */
public class StdNotIn extends AbstractInExpression<Column> {

    private static final long serialVersionUID = 2448932045855706827L;

    public StdNotIn(Criteria<?> criteria, Column column, Slot slot, Collection<?> values) {
    ...
    public static StdNotIn.Builder create() {
    ...
    public static final class Builder extends AbstractRangeExprBuilder<StdNotIn, Column> {
        ...
        @Override
        public StdNotIn build() {
            if (Objects.nonNull(this.target)) {
                return new StdNotIn(this.criteria, this.target, this.slot, this.values);
            }
            return null;
        }
    }
