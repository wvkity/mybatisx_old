 ...
 * the License.
 */
package com.github.mybatisx.spring.boot.batch.config;

import com.github.mybatisx.plugin.batch.AbstractBatchHandler;
import com.github.mybatisx.spring.boot.batch.MyBatisBatchAutoConfiguration;
...
public class MyBatisBatchConfigurer {
    ...
    public MyBatisBatchConfigurer(MyBatisBatchProperties batchProperties) {
        ...
        final List<String> methods = this.batchProperties.getInterceptMethods();
        if (!StringUtils.hasText(this.properties.getProperty(AbstractBatchHandler.PROP_KEY_BATCH_INTERCEPT_METHODS)) &&
            ...
            if (StringUtils.hasText(methodStr)) {
                this.properties.setProperty(AbstractBatchHandler.PROP_KEY_BATCH_INTERCEPT_METHODS, methodStr);
            }
        }
    }
