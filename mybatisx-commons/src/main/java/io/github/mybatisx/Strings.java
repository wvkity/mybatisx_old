...
public final class Strings {
     ...
     * @return 替换后的字符串
     */
    public static String replace(final String template, final Map<String, String> paramValues) {
        final Matcher matcher = PATTERN.matcher(template);
        final StringBuffer it = new StringBuffer();
        while (matcher.find()) {
            matcher.appendReplacement(it, paramValues.getOrDefault(matcher.group(2), ""));
        }
