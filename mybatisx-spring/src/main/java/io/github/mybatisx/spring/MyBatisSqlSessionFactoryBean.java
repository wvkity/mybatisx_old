...
import io.github.mybatisx.basic.keygen.DefaultKeyGenerator;
...
import io.github.mybatisx.spring.comparator.AnnotationAwareInterceptorOrderComparator;
import io.github.mybatisx.spring.transaction.MultiDataSourceSpringManagedTransactionFactory;
...
public class MyBatisSqlSessionFactoryBean implements
     ...
     */
    public void initFactory(final ApplicationContext context) {
    ...
    private void initBeanFactory() {
        try {
            if (this.applicationContext instanceof GenericApplicationContext) {
                final GenericApplicationContext ctx = (GenericApplicationContext) this.applicationContext;
                this.beanFactory = ctx.getDefaultListableBeanFactory();
                this.autowireBeanFactory = ctx.getAutowireCapableBeanFactory();
            }
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
        ...
        // CURD-SQL注入器
        if (this.globalConfiguration.getInjector() == null) {
            ifPresent(this.globalConfiguration::setInjector, Injector.class);
        }
        // 实体解析器
        if (this.globalConfiguration.getEntityParser() == null) {
            ifPresent(this.globalConfiguration::setEntityParser, EntityParser.class);
        }
        // 属性解析器
        if (this.globalConfiguration.getFieldParser() == null) {
            ifPresent(this.globalConfiguration::setFieldParser, FieldParser.class);
        }
        // 雪花ID生成器
        if (this.globalConfiguration.getSequence() == null) {
            ifPresent(this.globalConfiguration::setSequence, Sequence.class);
        }
        // ID生成器
        if (this.globalConfiguration.getKeyGenerator() == null) {
            ifPresent(this.globalConfiguration::setKeyGenerator, KeyGenerator.class);
        }
        if (this.globalConfiguration.getKeyGenerator() == null) {
            if (this.globalConfiguration.getSequence() == null) {
                this.globalConfiguration.setKeyGenerator(
                    new DefaultKeyGenerator(new DefaultSnowflakeSequence(SnowflakeConfig.millisSnowflakeConfig())));
            } else {
                this.globalConfiguration.setKeyGenerator(new DefaultKeyGenerator(this.globalConfiguration.getSequence()));
            }
        }
        // 缓存全局配置
        ...
        this.globalConfiguration.setFactory(factory);
        return factory;
    }

    private <T> MyBatisSqlSessionFactoryBean ifPresent(final Consumer<T> consumer, final Class<T> clazz) {
        if (hasBeanFromAware(clazz)) {
            try {
    ...
    private boolean hasBeanFromAware(final Class<?> clazz) {
        return Optional.ofNullable(this.applicationContext).map(it ->
            it.getBeanNamesForType(clazz, false, false).length > 0
        ).orElse(false);
    }

    private <T> T getBean(final Class<T> clazz) {
        return Optional.ofNullable(this.applicationContext).map(it -> it.getBean(clazz)).orElse(null);
    }
